...
- Sources: public iOS 18.1 MediaPlayer SDK headers and public reverse-engineered iOS 17.6.1 and iOS 18.2 MusicLibrary/MediaPlayer interfaces and implementations, including `MPMediaItem`, `MPMediaQuery`, `MPMediaPropertyPredicate`, `ML3Track`, `ML3Entity`, and `MPMediaLibraryDataProviderML3`.
...
- Use: verify the public `songsQuery` plus persistent-ID acceptance boundary and the `MLMediaTypeForMPMediaType` conversion performed before a public MediaPlayer media-type predicate is compared with the internal MusicLibrary track property.
- No Apple implementation source is copied or redistributed; the importer independently resolves the conversion symbol and available runtime interfaces.
