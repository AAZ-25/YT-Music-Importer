...
- No Apple implementation source is copied or redistributed. Beta 61 independently constructs a minimal MIP payload, submits it through the runtime-checked client-import session, then asks the returned Music record to perform Music's own local-file location transaction. It verifies the exact resolved path, readable playable audio, membership, metadata, and ownership ID before accepting the import.
