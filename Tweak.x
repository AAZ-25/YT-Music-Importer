...
static void YTMIScheduleBetaNotice(YTPlayerViewController *player) {
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.6 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        NSString *key = @"YTMusicImporterBeta55NoticeShown";
        ...
        UIAlertController *notice = [UIAlertController alertControllerWithTitle:@"YT Music Importer" message:@"Test build — Beta 55. Diagnostic logging is always on; no private song, path, account, or device data is written." preferredStyle:UIAlertControllerStyleAlert];
...
static void YTMIImportLibraryItem(NSDictionary *item, YTPlayerViewController *player) {
    ...
    NSString *importID = [NSString stringWithFormat:@"B55-%@", [rawID substringToIndex:8]];
...
%ctor {
    ...
    if (![defaults boolForKey:@"YTMusicImporterBeta55LogInitialized"]) {
        [NSFileManager.defaultManager removeItemAtPath:YTMILogPath() error:nil];
        [defaults setBool:YES forKey:@"YTMusicImporterBeta55LogInitialized"];
    }
    YTMILogStage(@"build.beta55.loaded");
