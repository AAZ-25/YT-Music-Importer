...
static void YTMIWriteResult(NSString *directory,NSString *nonce,BOOL success,NSInteger code){
    if(directory.length&&nonce.length)[@{@"nonce":nonce,@"success":@(success),@"code":@(code)}writeToFile:[directory stringByAppendingPathComponent:@"result.plist"]atomically:YES];
}
...
static void YTMIHandleRequest(void){
    @autoreleasepool{@synchronized(YTMIMusicDatabaseImporter.class){
        ...
        if(!nonce.length||!YTMIValidSource(audio,directory)||![NSFileManager.defaultManager fileExistsAtPath:audio]){if(nonce.length)YTMIWriteResult(directory,nonce,NO,43);return;}
        ...
        if(![fm copyItemAtPath:audio toPath:copy error:nil]){YTMIWriteResult(directory,nonce,NO,38);return;}
        ...
        YTMIQuiesceMusic();
        NSError *importError=nil;BOOL success=[[YTMIMusicDatabaseImporter new]importAudioAtURL:[NSURL fileURLWithPath:copy]metadata:metadata error:&importError];
        ...
        [fm removeItemAtPath:copy error:nil];[fm removeItemAtPath:pending error:nil];YTMIWriteResult(directory,nonce,success,code);
