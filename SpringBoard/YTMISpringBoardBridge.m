...
static void YTMIPresentMusicProgress(NSString *importID) {
    ...
    YTMIProgressAlert = [UIAlertController alertControllerWithTitle:@"YT Music Importer — Beta 47" message:[NSString stringWithFormat:@"%@\nPreparing local import\nElapsed: 00:00", importID] preferredStyle:UIAlertControllerStyleAlert];
    ...
    YTMIProgressTimer = [NSTimer scheduledTimerWithTimeInterval:1.0 repeats:YES block:^(__unused NSTimer *timer) {
        ...
        NSString *phase = elapsed < 3 ? @"Submitting direct import" : (elapsed < 48 ? @"Waiting for Music" : @"Verifying imported track");
        YTMIProgressAlert.message = [NSString stringWithFormat:@"%@\n%@\nElapsed: %02ld:%02ld\nMaximum: 00:55", importID, phase, (long)(elapsed / 60), (long)(elapsed % 60)];
    }];
}
...
static void YTMIHandleMusicRequest(void) {
    @autoreleasepool {@synchronized(YTMIMusicDatabaseImporter.class) {
        ...
        NSDictionary *job=[NSDictionary dictionaryWithContentsOfFile:pending]; NSString *nonce=[job[@"nonce"]isKindOfClass:NSString.class]?job[@"nonce"]:nil; NSString *audio=[job[@"audioPath"]isKindOfClass:NSString.class]?job[@"audioPath"]:nil; NSString *importID=[job[YTMIJobImportIDKey]isKindOfClass:NSString.class]?job[YTMIJobImportIDKey]:@"B47-UNKNOWN";
...
static void YTMIRelayFromSpringBoard(void) {
    @autoreleasepool {@synchronized(YTMIMusicDatabaseImporter.class) {
        NSString *directory=YTMIFindJobDirectory(); if(!directory.length)return; NSDictionary *job=[NSDictionary dictionaryWithContentsOfFile:[directory stringByAppendingPathComponent:@"pending.plist"]]; NSString *nonce=[job[@"nonce"]isKindOfClass:NSString.class]?job[@"nonce"]:nil; NSString *audio=[job[@"audioPath"]isKindOfClass:NSString.class]?job[@"audioPath"]:nil; NSString *importID=[job[YTMIJobImportIDKey]isKindOfClass:NSString.class]?job[YTMIJobImportIDKey]:@"B47-UNKNOWN"; NSMutableArray *trace=[NSMutableArray arrayWithObject:@"bridge.request.received"];
